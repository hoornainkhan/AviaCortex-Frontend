...
import { mockAircraft, mockMaintenanceRecords, mockPilotReports, mockUsers, mockTasks, mockFlights } from './data/mockData';
...
export default function App() {
  ...
  const engineers = useMemo(() => mockUsers.filter(u => u.role === 'engineer'), []);
  ...
  const handleSubmitTask = useCallback((taskId: string, updates: Partial<Task>) => {
    setTasks(prev => prev.map(task => 
      task.id === taskId ? { ...task, ...updates } : task
    ));
  }, []);

  const handleApproveTask = useCallback((taskId: string, managerNotes?: string) => {
    setTasks(prev => prev.map(task => 
      task.id === taskId 
  ...
  const handleRejectTask = useCallback((taskId: string, managerNotes: string) => {
    setTasks(prev => prev.map(task => 
      task.id === taskId 
  ...
  const handleSubmitWorkCard = useCallback((workCard: Partial<MaintenanceRecord>) => {
  ...
  const handleSubmitPilotReport = useCallback((report: Partial<PilotReport>) => {
  ...
  return (
    <div className="dark min-h-screen bg-background">
      ...
      <main className="container mx-auto p-6">
        ...
        {currentView === 'detail' && selectedAircraft && user.role === 'manager' && (
          <AircraftDetail
        ...
        {currentView === 'task-verification' && selectedTask && user.role === 'manager' && (
          <TaskVerification
            task={selectedTask}
            aircraft={mockAircraft.find(ac => ac.id === selectedTask.aircraftId) || mockAircraft[0]}
            engineerName={engineers.find(e => e.id === selectedTask.assignedTo)?.name || 'Unknown'}
            onBack={handleBack}
        ...
        {currentView === 'workcard' && selectedTask && user.role === 'engineer' && (
          <TaskWorkCard
            task={selectedTask}
            aircraft={mockAircraft.find(ac => ac.id === selectedTask.aircraftId) || mockAircraft[0]}
            onBack={handleBack}
